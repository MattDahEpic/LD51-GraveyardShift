...
public class Singleton<T> : MonoBehaviour where T : MonoBehaviour {
	...
	protected virtual void Awake() {
		if (FindObjectsOfType<T>().Length > 1) { //uh-oh, we got duplicated somewhere
			DestroyImmediate(gameObject); //destroy immediately so that OnDestroy runs right now and we can reset the destroyed flag.
			destroyed = false;
			return;
		}
	}
